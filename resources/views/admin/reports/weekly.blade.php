...
<div class="mb-4">
    <h1 class="page-title-new mb-1">
        <i class="bi bi-calendar-range-fill text-primary"></i> Weekly Sales Report
    </h1>
    <p class="text-muted small">Overview of sales performance for the current week</p>
</div>
...
<div class="card recent-sales-card border-0 shadow-sm mb-4">
    <div class="card-header bg-white border-bottom py-3">
        <h6 class="mb-0" style="font-weight: 700; color: #1e293b;">Daily Breakdown ({{ $startDate->format('d M Y') }} - {{ $endDate->format('d M Y') }})</h6>
    </div>
    <div class="card-body">
...
<div class="card recent-sales-card border-0 shadow-sm">
    <div class="card-header bg-white border-bottom py-3">
        <h6 class="mb-0" style="font-weight: 700; color: #1e293b;">Sales by Day</h6>
    </div>
            ...
            <table class="table products-table mb-0">
                ...
                <tbody>
                    @forelse($dailyData as $day)
                        <tr>
                            <td class="fw-600 color-navy">{{ \Carbon\Carbon::createFromFormat('Y-m-d', $day->date)->format('d M Y (l)') }}</td>
                            <td>{{ $day->count }}</td>
                            <td class="fw-700 color-navy">₵{{ number_format($day->revenue, 2) }}</td>
                            <td class="text-end text-success fw-700">₵{{ number_format($day->profit, 2) }}</td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="4" class="text-center py-5 text-muted small">No sales recorded this week.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
